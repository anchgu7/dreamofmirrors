...
public class RotationRing : MonoBehaviour {
	...
	void Update () {
		if (!GameFields.paused) {
			...
			if (hasTarget) {
				transform.rotation = Quaternion.Euler(0.0f, 0.0f, 5 + (180/Mathf.PI) * Mathf.Atan2((target.transform.position.y - transform.position.y), (target.transform.position.x - transform.position.x)) + 90);
				var parentTransform = transform.parent.GetComponent<Transform>();
				parentTransform.rotation = Quaternion.Euler(0.0f, 0.0f, 5 + (180/Mathf.PI) * Mathf.Atan2((target.transform.position.y - transform.position.y), (target.transform.position.x - transform.position.x)) + 90);
			}
